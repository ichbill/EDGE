...
def load_from_timm(model_name, pretrained):
    if model_name == 'clip':
        ...
        # Load NFNet model - requires pretrained weights file
        pretrained_path = os.path.join('utils', 'pretrained_encoders', 'nfnet_l0_ra2-45c6688d.pth')
        model = timm.create_model('nfnet_l0', pretrained=pretrained, num_classes=0, global_pool="avg",
                                        pretrained_cfg_overlay=dict(file=pretrained_path) if os.path.exists(pretrained_path) else None)
    elif model_name == 'vit':
