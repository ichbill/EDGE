...
def epoch_test(dataloader, model, device, bert_test_embed):
    ...
    # print('train text_embeds.shape: ', text_embeds.shape)
...
@torch.no_grad()
def epoch_test_cc3m(dataloader, model, device, bert_test_embed):
    model.eval() 
    logit_scale = model.logit_scale.detach()
    start_time = time.time()  
    ...
    use_image_projection = False
    if use_image_projection:
        image_embeds = model.image_projection(image_embeds.float())
        image_embeds = image_embeds / image_embeds.norm(dim=1, keepdim=True)
    else:
        image_embeds = image_embeds / image_embeds.norm(dim=1, keepdim=True)
    ...
    score_matrix_i2t = torch.full((len(image_embeds), len(text_embeds)), -100.0).to(device)
    
    for i in range(0, len(image_embeds), chunk_size):
        image_chunk = image_embeds[i:i+chunk_size]
        sims = logit_scale.exp() * image_chunk @ text_embeds.t()  # Compute similarities for chunk

        # Select top-k values and indices for each image in the chunk
        for j in range(sims.size(0)):  # Loop over each image in the current chunk
            topk_sim, topk_idx = sims[j].topk(k=128, dim=0)
            score_matrix_i2t[i + j, topk_idx] = topk_sim

    # Transpose similarity matrix for text-to-image
    score_matrix_t2i = torch.full((len(text_embeds), len(image_embeds)), -100.0).to(device)
    for i in range(0, len(text_embeds), chunk_size):
        text_chunk = text_embeds[i:i+chunk_size]
        sims = logit_scale.exp() * text_chunk @ image_embeds.t()

        # Select top-k values and indices for each text in the chunk
        for j in range(sims.size(0)):
            topk_sim, topk_idx = sims[j].topk(k=128, dim=0)
            score_matrix_t2i[i + j, topk_idx] = topk_sim
